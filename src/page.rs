...
fn show_sig(header: &str, sig: Option<git2::Signature>) -> String {
    let sig = match sig {
        Some(s) => s,
...
fn show_commit(commit: &git2::Commit) -> String {
    let mut response = String::new();
    write!(&mut response, "tree {}\n", commit.tree_id()).unwrap_or_default();
    for parent in commit.parent_ids() {
        write!(&mut response, "parent {}\n", parent).unwrap_or_default();
    }
    write!(
        &mut response,
        "{}\n",
...
fn show_tag(tag: &git2::Tag) -> String {
    let mut response = String::new();
    write!(&mut response, "object {}\n", tag.target_id()).unwrap_or_default();
    write!(&mut response, "type {}\n", tag.target_type().unwrap().str()).unwrap_or_default();
    write!(&mut response, "tag {}\n", tag.name().unwrap()).unwrap_or_default();
...
fn show_tree(tree: &git2::Tree) -> String {
    let mut response = String::new();
...
//     let target = target.unwrap_or("");
//     if target == "info" {
//         return objects_info(name, project_name, Some(target), rest);
//     }
//     let rest = rest.unwrap_or(&[]);
//     if rest.len() != 1 {
//         return Err(Missing::Nowhere);
//     }
//     let id = format!("{}{}", target, rest[0]);
//     let repo = get_repo(name, project_name)?;
//     // let data = repo.odb_object(id)?;
//     let object: git2::Object = repo.object(id)?;
//     let content = match object.kind() {
//         Some(git2::ObjectType::Blob) => std::str::from_utf8(object.as_blob().unwrap().content())
//             .unwrap_or("")
//             .to_string(),
//         Some(git2::ObjectType::Commit) => {
//             let response = show_commit(object.as_commit().unwrap());
//             response
//         }
//         Some(git2::ObjectType::Tag) => {
//             let response = show_tag(object.as_tag().unwrap());
//             response
//         }
//         Some(git2::ObjectType::Tree) => {
//             let response = show_tree(object.as_tree().unwrap());
//             response
//         }
//         Some(git2::ObjectType::Any) | None => {
//             return Err(Missing::Nowhere);
//         }
//     };
...
//     let data = format!(
//         "commit 197{}{}",
//         std::char::from_u32(0x0000).unwrap(),
//         content
//     );
//     std::io::copy(&mut data.as_bytes(), &mut encoder).unwrap();
//     let encoded_data = encoder.finish().into_result().unwrap();
//     Ok(ContentType::Binary("text/plain".to_string(), encoded_data))
// }
